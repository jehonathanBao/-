    ...
    <section className="console-panel mb-5 p-4 md:p-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-400">
            只监控 Binance USDT 永续山寨合约，默认排除 BTC / ETH；用于识别主力建仓、异常拉砸、吸收、压制和清算瀑布。
          </p>
        </div>
        <div className="grid grid-cols-2 gap-2 text-xs text-slate-300 md:grid-cols-4 xl:grid-cols-6">
          ...
          <StatusPill label="监控币种" value={summary.monitoredSymbols.length || 0} tone="cyan" />
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr)_260px]">
        <div className="console-panel-muted p-3 text-xs text-slate-400">
          ...
          <p className="mt-1">
            {summary.monitoredSymbols.length > 0
              ? summary.monitoredSymbols.join(", ")
              : "未启用或暂无山寨合约列表"}
          </p>
          <p className="mt-2 text-cyan-200">Candidate only · 只读提醒 · 不下单 · dry-run 默认开启</p>
        </div>
...
function RuntimeSummary({ summary }) {
  const chips = [
    ...
    ["ForceOrder", summary.lastForceOrderAt ? relativeAge(summary.lastForceOrderAt) : "暂无"],
  ...
  return (
    <div className="mt-3 grid gap-3 text-xs lg:grid-cols-2">
      ...
      <div className="console-panel-muted p-3 lg:col-span-2">
        ...
        <p className="mt-2 text-slate-400">
          {universe.mode || "auto"} · limit {universe.limit || 0} · min 24h {formatUsd(universe.min24hQuoteVolumeUsd || 0)}
        </p>
        <p className="mt-1 text-slate-500">
          whitelist {(universe.whitelist || []).slice(0, 8).join(", ") || "auto"} · blacklist {(universe.blacklist || []).join(", ") || "none"} · excluded {(universe.excludedSymbols || []).join(", ") || "none"}
        </p>
      </div>
    </div>
  );
