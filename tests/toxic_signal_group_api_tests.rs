...
#[tokio::test]
async fn toxic_signal_group_api_returns_read_only_group_payloads() {
    ...
    let status = test_http_get(format!("http://{addr}/api/toxicity/signal-groups/status"))
        .await
    ...
    let recent = test_http_get(format!("http://{addr}/api/toxicity/signal-groups/recent"))
        .await
    ...
    assert!(recent_payload["groups"].is_array());
    assert!(recent_payload["groups"]
        .as_array()
        .expect("groups array")
        .iter()
        .all(|group| group["readOnly"] == true
            && group["runtimeModified"] == false
            && group["analysisOnly"] == true
            && group["executionEnabled"] == false
            && group["originalSignalsPreserved"] == true));

    let by_symbol = test_http_get(format!(
        "http://{addr}/api/toxicity/signal-groups/{}",
        "ETH-PERP"
    ))
    .await
    ...
    assert!(by_symbol_payload["groups"]
        .as_array()
        .expect("symbol groups")
        .is_empty());

    let filtered = test_http_get(format!(
        "http://{addr}/api/toxicity/signal-groups/recent?symbol=%20btc-perp%20"
    ))
    .await
    ...
    let detail = test_http_get(format!(
        "http://{addr}/api/toxicity/signal-groups/group/missing-group"
    ))
    .await
    .expect("group response");
    assert_eq!(detail.status(), reqwest::StatusCode::OK);
    let detail_payload: serde_json::Value = detail.json().await.expect("detail json");
