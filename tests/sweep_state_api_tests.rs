...
#[tokio::test]
async fn sweep_state_api_returns_empty_windows_and_quality() {
    ...
    let response = test_http_get(format!("http://{addr}/api/sweep-state"))
        .await
        .expect("response");
    ...
    assert_eq!(payload["windowsMs"], serde_json::json!([1000, 5000, 15000]));
    assert!(payload["results"]["1000"].is_object());
    assert!(payload["results"]["5000"].is_object());
    assert!(payload["results"]["15000"].is_object());
    assert_eq!(payload["results"]["1000"]["direction"], "none");
    assert_eq!(payload["results"]["1000"]["sweepDetected"], false);
...
#[test]
fn status_response_includes_sweep_summary() {
    let state = AppState::new(test_config());
