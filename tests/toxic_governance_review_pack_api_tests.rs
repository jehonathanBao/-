...
#[tokio::test]
async fn toxic_governance_review_pack_api_returns_review_pack_only_payloads() {
    ...
    let status = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-review-pack/status"
    ))
    .await
    ...
    let summary = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-review-pack/summary"
    ))
    .await
    ...
    let by_symbol = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-review-pack/{}",
        "BTC-PERP"
    ))
    .await
    ...
    assert_eq!(by_symbol_payload["reviewPackOnly"], true);

    let export = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-review-pack/export"
    ))
    .await
