...
#[tokio::test]
async fn manual_apply_governance_api_serves_json_and_markdown() {
    let report_dir = temp_report_dir("manual_governance_api_ready");
    write_ready_calibration_report(&report_dir, "calibration-9800", 9_800);
    approve_all_latest_recommendations(&report_dir, 9_810);
    create_export(&report_dir);
    create_approved_signoff(&report_dir, chrono::Local::now().timestamp_millis());

    let state = AppState::new(test_config(report_dir.clone(), true));
    ...
    let payload = test_http_get(format!("http://{addr}/api/parameter-review/governance"))
        .await
    ...
    assert_eq!(payload["applyMode"], "governance_index_only");
    assert_eq!(
        payload["governanceStatus"],
        "READY_FOR_EXTERNAL_MANUAL_APPLY"
    );
    assert!(payload["nextRequiredAction"]
        .as_str()
        .expect("next action")
        .contains("runbook"));
    assert!(payload["links"]["startupCheck"].is_string());
    assert!(payload["links"]["signoffStatus"].is_string());
    assert!(payload["links"]["evidenceFreshness"].is_string());
    assert!(payload["links"]["auditStory"].is_string());
    assert!(payload["safetyBoundary"]
        .as_array()
        .is_some_and(|items| !items.is_empty()));

    let latest = test_http_get(format!(
        "http://{addr}/api/parameter-review/governance/latest"
    ))
    .await
    .expect("governance latest")
    .json::<serde_json::Value>()
    .await
    .expect("governance latest json");
    assert_eq!(latest["applyMode"], "governance_index_only");

    let markdown = test_http_get(format!("http://{addr}/api/parameter-review/governance.md"))
        .await
        .expect("governance markdown");
    assert_eq!(markdown.status(), reqwest::StatusCode::OK);
    let markdown_text = markdown.text().await.expect("markdown text");
    assert!(markdown_text.contains("Manual Apply Governance Index"));
    assert!(markdown_text.contains("Read-only: true"));
    assert!(markdown_text.contains("Runtime modified: false"));
    assert!(markdown_text.contains("Apply mode: governance_index_only"));

    server.abort();
}

#[tokio::test]
async fn manual_apply_governance_api_surfaces_blocking_reasons_when_signoff_is_missing() {
    let report_dir = temp_report_dir("manual_governance_api_blocked");
    write_ready_calibration_report(&report_dir, "calibration-9810", 9_810);
    approve_all_latest_recommendations(&report_dir, 9_820);
    create_export(&report_dir);
    ...
    let payload = test_http_get(format!("http://{addr}/api/parameter-review/governance"))
        ...
        .await
        .expect("governance json");
    assert_eq!(payload["governanceStatus"], "BLOCKED_BY_SIGNOFF");
...
fn create_approved_signoff(report_dir: &Path, now_ms: i64) {
    ManualSignoffStore::new(
        ...
        test_config(report_dir.to_path_buf(), true),
    )
    .create_signoff_at(
        ManualSignoffInput {
...
fn create_export(report_dir: &Path) {
    ManualParameterExportStore::new(report_dir)
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("governance api export".to_string()),
        })
...
fn test_config(replay_report_dir: PathBuf, read_only: bool) -> AppConfig {
    AppConfig {
        ...
        replay_enabled: true,
        replay_report_dir: replay_report_dir.display().to_string(),
        vpin_enabled: true,
        vpin_bucket_size_btc: 100.0,
        vpin_lookback_buckets: 50,
        vpin_min_buckets: 10,
