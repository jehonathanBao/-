...
#[tokio::test]
async fn toxic_quality_scorecard_api_returns_analysis_only_payloads() {
    ...
    let status = test_http_get(format!(
        "http://{addr}/api/toxicity/quality-scorecard/status"
    ))
    .await
    ...
    let summary = test_http_get(format!(
        "http://{addr}/api/toxicity/quality-scorecard/summary"
    ))
    .await
    ...
    assert!(summary_payload["byWindow"].is_array());
    assert!(summary_payload["downgradeCandidates"].is_array());
    assert!(summary_payload["noTradeCandidates"].is_array());

    let by_symbol = test_http_get(format!(
        "http://{addr}/api/toxicity/quality-scorecard/{}",
        "BTC-PERP"
    ))
    .await
    ...
    assert_eq!(by_symbol_payload["mode"], "analysis_only");
