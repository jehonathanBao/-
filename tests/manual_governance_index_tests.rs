...
use btc_toxic_flow_monitor_rs::{
    calibration::{
        ...
        manual_governance_index::ManualApplyGovernanceIndexStore,
...
#[test]
fn manual_governance_index_reports_ready_state() {
    let report_dir = temp_report_dir("manual_governance_ready");
    write_ready_calibration_report(&report_dir, "calibration-9900", 9_900);
    approve_all_latest_recommendations(&report_dir, 9_910);
    ...
    assert!(index.current_blocker.is_none());
    assert!(index.blocking_reasons.is_empty());
    assert_eq!(index.next_owner, "External Manual Executor");
    assert!(index.next_action.contains("runbook"));
}

#[test]
fn manual_governance_index_blocks_when_signoff_is_missing() {
    let report_dir = temp_report_dir("manual_governance_missing_signoff");
    write_ready_calibration_report(&report_dir, "calibration-9910", 9_910);
    approve_all_latest_recommendations(&report_dir, 9_920);
    create_export(&report_dir);
    ...
    assert_eq!(index.current_blocker.as_deref(), Some("SIGNOFF_MISSING"));
    assert!(index
        .blocking_reasons
        .iter()
        .any(|issue| issue.code == "SIGNOFF_MISSING"));
    assert!(index.next_action.contains("sign-off") || index.next_action.contains("sign off"));
}
