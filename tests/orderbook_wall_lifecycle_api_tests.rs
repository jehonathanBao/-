...
#[tokio::test]
async fn orderbook_wall_lifecycle_api_returns_analysis_only_payloads() {
    ...
    let status = test_http_get(format!("http://{addr}/api/toxicity/orderbook-walls/status"))
        .await
    ...
    assert_eq!(status_payload["analysisMode"], "analysis_only");

    let recent = test_http_get(format!("http://{addr}/api/toxicity/orderbook-walls/recent"))
        .await
    ...
    assert_eq!(recent_payload["analysisMode"], "analysis_only");
    assert!(recent_payload["trackedWalls"].is_array());
    assert!(recent_payload["recentEvents"].is_array());
    assert!(recent_payload["toxicityCandidates"].is_array());

    let by_symbol = test_http_get(format!(
        "http://{addr}/api/toxicity/orderbook-walls/{}",
        "BTC-PERP"
    ))
    .await
