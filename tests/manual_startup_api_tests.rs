...
#[tokio::test]
async fn manual_startup_check_api_returns_ready_when_chain_passes() {
    let report_dir = temp_report_dir("manual_startup_api_ready");
    write_ready_calibration_report(&report_dir, "calibration-9030", 9_030);
    approve_all_latest_recommendations(&report_dir, 9_040);
    ManualParameterExportStore::new(&report_dir)
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("startup api test".to_string()),
        })
    ...
    let response = test_http_get(format!(
        "http://{addr}/api/calibration/manual-startup/check"
    ))
    .await
    .expect("startup check")
    .json::<serde_json::Value>()
    .await
    .expect("startup check json");

    assert_eq!(response["status"], "READY_FOR_MANUAL_APPLY");
...
#[tokio::test]
async fn manual_startup_check_api_returns_missing_report_when_absent() {
    let report_dir = temp_report_dir("manual_startup_api_missing_report");
    let state = AppState::new(test_config(report_dir));
    let app = router(state);
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0")
        .await
    ...
    assert_eq!(response["status"], "MISSING_REPORT");
    assert_eq!(response["readOnly"], true);
