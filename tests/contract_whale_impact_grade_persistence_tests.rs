...
fn assessment() -> ContractEventImpactAssessment {
    let mut episode = ContractImpactEpisode {
        ...
        peak_window_volume_btc: 3_000.0,
...
#[test]
fn v32_alias_map_resolves_episode_projection_and_source_ids() {
    let path =
        std::env::temp_dir().join(format!("impact-grade-alias-{}.sqlite", std::process::id()));
    let _ = std::fs::remove_file(&path);
    let store = SqliteStore::open(path.to_str().unwrap()).unwrap();
    store.migrate().unwrap();
    let repo = ContractEventGradeRepo::new(store);
    let value = assessment();
    repo.upsert_assessment_with_aliases(
        &value,
        &[
            (value.episode_id.as_str(), Some("projection-1"), None),
            (
                value.event_id.as_str(),
                Some("projection-1"),
                Some(value.event_id.as_str()),
            ),
        ],
        1_700_000_100_000,
    )
    .unwrap();
    assert_eq!(
        repo.get_assessment_for_aliases(&["projection-1"], &value.grade_version)
            .unwrap()
            .unwrap()
            .episode_id,
        value.episode_id
    ...
    assert_eq!(
        repo.get_assessment_for_aliases(&[value.event_id.as_str()], &value.grade_version)
            .unwrap()
            .unwrap()
            .grade,
        value.grade
...
#[test]
fn one_aggregated_episode_can_materialize_multiple_source_events() {
    let path = std::env::temp_dir().join(format!(
        "impact-grade-episode-sources-{}.sqlite",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&path);
    let store = SqliteStore::open(path.to_str().unwrap()).unwrap();
    ...
    second.event_id = "event-persist-2".to_string();
    repo.upsert_assessment(&first, 1_700_000_100_000).unwrap();
    repo.upsert_assessment(&second, 1_700_000_100_001).unwrap();
    let count: i64 = store
        .with_connection(|conn| {
            Ok(conn.query_row(
                "SELECT COUNT(*) FROM contract_event_impact_grades WHERE episode_id = ?1 AND grade_version = ?2",
                [&first.episode_id, &first.grade_version],
                |row| row.get(0),
            )?)
        })
        .unwrap();
    assert_eq!(count, 2);
    assert!(!repo
        .episode_alert_already_sent(&first.episode_id, &first.grade_version)
        .unwrap());
    assert!(repo
        .mark_episode_alert_sent(&first.episode_id, &first.grade_version, 1_700_000_100_002,)
        .unwrap());
    assert!(repo
