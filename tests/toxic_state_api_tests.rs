...
use btc_toxic_flow_monitor_rs::{
    api::{routes::build_status_response, server::router},
    app::AppState,
    config::{
...
#[tokio::test]
async fn toxic_state_api_returns_empty_results() {
    ...
    let response = test_http_get(format!("http://{addr}/api/toxic-state"))
        .await
        .expect("response");
    ...
    assert_eq!(payload["thresholdBtc"], 1000.0);
    assert_eq!(
        payload["windowsMs"],
        serde_json::json!([1000, 5000, 15000, 60000])
    );
    assert!(payload["results"].as_object().is_some());
    assert_eq!(payload["results"].as_object().unwrap().len(), 0);
...
#[test]
fn status_response_includes_toxic_summary() {
    let state = AppState::new(test_config());
    let status = build_status_response(&state);

    assert!(status.toxic.enabled);
    assert_eq!(status.toxic.threshold_btc, 1000.0);
    assert_eq!(status.toxic.latest_toxic_volume_btc, 0.0);
    assert_eq!(status.toxic.recent_event_count, 0);
