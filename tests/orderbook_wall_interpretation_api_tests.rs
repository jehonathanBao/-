...
#[tokio::test]
async fn orderbook_wall_interpretation_api_returns_analysis_only_payloads() {
    ...
    let status = test_http_get(format!(
        "http://{addr}/api/toxicity/orderbook-wall-interpretation/status"
    ))
    .await
    ...
    let recent = test_http_get(format!(
        "http://{addr}/api/toxicity/orderbook-wall-interpretation/recent"
    ))
    .await
    ...
    let by_symbol = test_http_get(format!(
        "http://{addr}/api/toxicity/orderbook-wall-interpretation/{}",
        "BTC-PERP"
    ))
    .await
