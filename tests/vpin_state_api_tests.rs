...
#[tokio::test]
async fn vpin_state_api_returns_empty_state() {
    ...
    let response = test_http_get(format!("http://{addr}/api/vpin-state"))
        .await
        .expect("response");
    assert_eq!(response.status(), reqwest::StatusCode::OK);
    let payload: serde_json::Value = response.json().await.expect("json");
    assert_eq!(payload["symbol"], "BTC-PERP");
    assert_eq!(payload["metrics"]["enabled"], true);
    assert_eq!(payload["metrics"]["activeBucketProgressBtc"], 0.0);

    let buckets = test_http_get(format!("http://{addr}/api/vpin-buckets?limit=20"))
        .await
        .expect("buckets response");
    let buckets_payload: serde_json::Value = buckets.json().await.expect("buckets json");
    assert!(buckets_payload["buckets"].is_array());

    server.abort();
}

#[test]
fn status_response_includes_vpin_summary() {
    let state = AppState::new(test_config());
