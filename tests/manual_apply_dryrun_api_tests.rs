...
#[tokio::test]
async fn manual_apply_dryrun_api_serves_json_and_markdown() {
    let report_dir = temp_report_dir("manual_apply_dryrun_api");
    write_calibration_report(&report_dir, "calibration-8850", 8_850);
    ...
    let export_id = ManualParameterExportStore::new(&report_dir)
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("dry-run api test".to_string()),
        })
    ...
    let latest = test_http_get(format!(
        "http://{addr}/api/parameter-review/exports/latest/dry-run"
    ))
    .await
    .expect("latest dry-run")
    .json::<serde_json::Value>()
    .await
    .expect("latest dry-run json");
    assert_eq!(latest["applyMode"], "dry_run_only");
    assert_eq!(latest["runtimeModified"], false);
    assert_eq!(latest["exportId"], export_id);

    let markdown = test_http_get(format!(
        "http://{addr}/api/parameter-review/exports/{export_id}/dry-run.md"
    ))
    .await
    .expect("dry-run markdown");
    assert_eq!(markdown.status(), reqwest::StatusCode::OK);
    let markdown_text = markdown.text().await.expect("markdown text");
    assert!(markdown_text.contains("# Manual Apply Dry-run Report"));
    assert!(markdown_text.contains("Runtime Safety Confirmation"));

    server.abort();
}

#[tokio::test]
async fn manual_apply_dryrun_api_reports_missing_exports_cleanly() {
    let report_dir = temp_report_dir("manual_apply_dryrun_api_empty");
    write_calibration_report(&report_dir, "calibration-8860", 8_860);
    ...
    assert_eq!(latest_json["applyMode"], "dry_run_only");
    assert_eq!(latest_json["runtimeModified"], false);

    let missing = test_http_get(format!(
        "http://{addr}/api/parameter-review/exports/does-not-exist/dry-run.md"
    ))
    .await
    .expect("missing dry-run markdown");
    assert_eq!(missing.status(), reqwest::StatusCode::NOT_FOUND);
    let text = missing.text().await.expect("missing dry-run text");
