...
use btc_toxic_flow_monitor_rs::{
    ...
    calibration::{
        ...
        parameter_recommendation_review_store::{ReviewDecisionInput, ReviewStatus},
    },
    config::{
...
#[tokio::test]
async fn manual_export_api_creates_and_lists_manual_only_exports() {
    let report_dir = temp_report_dir("manual_export_api");
    write_calibration_report(&report_dir, "calibration-6000", 6_000);

    let state = AppState::new(test_config(report_dir.to_string_lossy().to_string()));
    let app = router(state.clone());
    let review_store =
        btc_toxic_flow_monitor_rs::calibration::parameter_recommendation_review_store::ParameterRecommendationReviewStore::new(&report_dir);
    ...
    review_store
        .append_review(
            ReviewDecisionInput {
                ...
                reviewer_note: Some("approved for export".to_string()),
    ...
    let create = client
        .post(format!("http://{addr}/api/parameter-review/exports"))
        .json(&serde_json::json!({
            "include_statuses": ["ApprovedForManualApply"],
            "operator": "manual",
            "note": "R14 smoke export"
        }))
        .send()
        .await
        ...
        .await
        .expect("create export json");
    assert_eq!(create["applyMode"], "manual_only");
    assert_eq!(create["runtimeModified"], false);
    assert_eq!(create["exportCreated"], true);

    let list = test_http_get(format!("http://{addr}/api/parameter-review/exports"))
        .await
        .expect("list exports")
        .json::<serde_json::Value>()
        .await
        .expect("list json");
    assert_eq!(list["exports"][0]["applyMode"], "manual_only");

    let latest = test_http_get(format!("http://{addr}/api/parameter-review/exports/latest"))
        .await
        .expect("latest export")
        .json::<serde_json::Value>()
        .await
        .expect("latest json");
    assert_eq!(latest["export"]["summary"]["runtimeModified"], false);
    assert_eq!(latest["export"]["export"]["safety"]["manualOnly"], true);

    server.abort();
}

#[tokio::test]
async fn manual_export_api_returns_no_approved_recommendations_when_empty() {
    let report_dir = temp_report_dir("manual_export_api_empty");
    write_calibration_report(&report_dir, "calibration-7000", 7_000);
    let state = AppState::new(test_config(report_dir.to_string_lossy().to_string()));
    ...
    let create = client
        ...
        .await
        .expect("create export json");
    assert_eq!(create["exportCreated"], false);
    assert_eq!(create["reason"], "no_approved_recommendations");
    assert_eq!(create["runtimeModified"], false);
