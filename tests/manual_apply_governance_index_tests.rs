...
use btc_toxic_flow_monitor_rs::{
    calibration::{
        ...
        manual_apply_governance_index::{
            derive_governance_status, ManualApplyGovernanceIndexStore,
        },
        manual_evidence_freshness::ManualEvidenceFreshnessStatus,
        manual_parameter_export::{ManualParameterExportRequest, ManualParameterExportStore},
        manual_signoff_store::ManualSignoffGateStatus,
        manual_signoff_store::{ManualSignoffDecision, ManualSignoffInput, ManualSignoffStore},
        manual_startup_check::ManualStartupStatus,
...
#[test]
fn governance_index_reports_ready_for_external_manual_apply() {
    let report_dir = temp_report_dir("governance_ready");
    write_ready_calibration_report(&report_dir, "calibration-9700", 9_700);
    approve_all_latest_recommendations(&report_dir, 9_710);
    create_export(&report_dir);
    create_approved_signoff(&report_dir, chrono::Local::now().timestamp_millis());

    let index = ManualApplyGovernanceIndexStore::new(report_dir, test_config(true))
        .build_index()
        .expect("governance index");

    assert!(index.read_only);
    assert!(!index.runtime_modified);
    assert_eq!(index.apply_mode, "governance_index_only");
    assert_eq!(index.governance_status, "READY_FOR_EXTERNAL_MANUAL_APPLY");
    assert_eq!(index.final_gate, "READY");
    assert_eq!(index.readiness_status, "READY_FOR_MANUAL_APPLY");
    assert_eq!(index.signoff_status, "SIGNED_OFF");
    assert_eq!(index.freshness_status, "FRESH");
    assert_eq!(index.audit_story_status, "AVAILABLE");
    assert!(index.blocking_issues.is_empty());
    assert!(index
        .safety_boundary
        .iter()
        .any(|item| item.contains("No calibration runner was triggered")));
}

#[test]
fn governance_index_blocks_by_readiness_when_read_only_is_false() {
    let report_dir = temp_report_dir("governance_readiness_blocked");
    write_ready_calibration_report(&report_dir, "calibration-9710", 9_710);

    let index = ManualApplyGovernanceIndexStore::new(report_dir, test_config(false))
        .build_index()
        .expect("governance index");

    assert!(index.read_only);
    assert!(!index.runtime_modified);
    assert_eq!(index.governance_status, "BLOCKED_BY_READINESS");
    assert!(!index.blocking_issues.is_empty());
}

#[test]
fn governance_index_blocks_by_signoff_when_missing() {
    let report_dir = temp_report_dir("governance_signoff_missing");
    write_ready_calibration_report(&report_dir, "calibration-9720", 9_720);
    approve_all_latest_recommendations(&report_dir, 9_730);
    create_export(&report_dir);
    ...
    assert_eq!(index.governance_status, "BLOCKED_BY_SIGNOFF");
    assert!(index
        .blocking_issues
        .iter()
        .any(|issue| issue.code == "SIGNOFF_MISSING"));
}

#[test]
fn governance_index_blocks_by_stale_evidence() {
    let report_dir = temp_report_dir("governance_stale");
    write_ready_calibration_report(&report_dir, "calibration-9730", 9_730);
    approve_all_latest_recommendations(&report_dir, 9_740);
    create_export(&report_dir);
    create_approved_signoff(&report_dir, 9_750);
    append_approved_review_update(&report_dir, 9_760);

    let index = ManualApplyGovernanceIndexStore::new(report_dir, test_config(true))
        .build_index()
        .expect("governance index");

    assert_eq!(index.governance_status, "BLOCKED_BY_STALE_EVIDENCE");
    assert!(index
        .blocking_issues
        .iter()
        .any(|issue| issue.code == "EVIDENCE_STALE"));
}

#[test]
fn governance_index_blocks_by_expired_signoff() {
    let report_dir = temp_report_dir("governance_expired");
    write_ready_calibration_report(&report_dir, "calibration-9740", 9_740);
    approve_all_latest_recommendations(&report_dir, 9_750);
    create_export(&report_dir);
    let old_ts = chrono::Local::now().timestamp_millis() - (31 * 60 * 1000);
    create_approved_signoff(&report_dir, old_ts);

    let index = ManualApplyGovernanceIndexStore::new(report_dir, test_config(true))
        .build_index()
        .expect("governance index");

    assert_eq!(index.governance_status, "BLOCKED_BY_EXPIRED_SIGNOFF");
    assert!(index
        .blocking_issues
        .iter()
        .any(|issue| issue.code == "SIGNOFF_EXPIRED" || issue.code == "EVIDENCE_EXPIRED"));
}

#[test]
fn governance_index_blocks_by_missing_evidence_when_report_is_missing() {
    let report_dir = temp_report_dir("governance_missing_evidence");

    let index = ManualApplyGovernanceIndexStore::new(report_dir, test_config(true))
        .build_index()
        .expect("governance index");

    assert_eq!(index.governance_status, "BLOCKED_BY_MISSING_EVIDENCE");
}

#[test]
fn governance_status_helper_blocks_when_audit_story_is_unavailable() {
    let status = derive_governance_status(
        ManualStartupStatus::ReadyForManualApply,
        ManualSignoffGateStatus::SignedOff,
        ManualEvidenceFreshnessStatus::Fresh,
        false,
    );
    assert_eq!(status, "BLOCKED_BY_AUDIT_STORY");
}

fn create_approved_signoff(report_dir: &Path, now_ms: i64) {
    ManualSignoffStore::new(report_dir.to_path_buf(), test_config(true))
        .create_signoff_at(
            ManualSignoffInput {
...
fn create_export(report_dir: &Path) {
    ManualParameterExportStore::new(report_dir)
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("governance export".to_string()),
        })
...
fn append_approved_review_update(report_dir: &Path, now_ms: i64) {
    let review_store = ParameterRecommendationReviewStore::new(report_dir);
    if let Some(card) = review_store
        ...
        review_store
            .append_review(
                ReviewDecisionInput {
                    ...
                    report_id: card.report_id,
                    status: ReviewStatus::ApprovedForManualApply,
                    reviewer_note: Some("reapproved".to_string()),
                    reviewer: Some("codex".to_string()),
                },
                now_ms,
            )
            .expect("append review update");
    }
}
...
fn test_config(read_only: bool) -> AppConfig {
    AppConfig {
