...
#[tokio::test]
async fn toxic_governance_signoff_pack_api_returns_signoff_pack_only_payloads() {
    let state = AppState::new(test_config());
    state
        .flow_service_for_tests()
        .add_book_for_tests(book_snapshot(1_000, 100_000.0));
    state
        .flow_service_for_tests()
        .add_book_for_tests(book_snapshot(61_000, 99_900.0));
    ...
    let status = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-signoff-pack/status"
    ))
    .await
    ...
    assert_eq!(status_payload["proposalOnly"], true);
    assert_eq!(status_payload["reviewPackOnly"], true);
    assert_eq!(status_payload["signoffPackOnly"], true);
    ...
    let summary = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-signoff-pack/summary"
    ))
    .await
    ...
    assert_eq!(summary_payload["proposalOnly"], true);
    assert_eq!(summary_payload["reviewPackOnly"], true);
    assert_eq!(summary_payload["signoffPackOnly"], true);
    assert!(summary_payload["byAction"].is_object());
    assert!(summary_payload["byDecision"].is_object());
    assert!(summary_payload["blockedReasons"].is_array());
    assert!(summary_payload["items"].is_array());

    let by_symbol = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-signoff-pack/{}",
        "BTC-PERP"
    ))
    .await
    ...
    assert_eq!(by_symbol_payload["signoffPackOnly"], true);

    let export = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-signoff-pack/export"
    ))
    .await
    ...
    assert_eq!(export_payload["proposalOnly"], true);
    assert_eq!(export_payload["reviewPackOnly"], true);
    assert_eq!(export_payload["signoffPackOnly"], true);
...
fn book_snapshot(ts: i64, mid: f64) -> NormalizedBook {
    NormalizedBook {
        venue: Venue::Binance,
        ...
        best_bid: mid - 10.0,
        best_ask: mid + 10.0,
        bids: vec![(mid - 10.0, 2.0)],
        asks: vec![(mid + 10.0, 2.0)],
        mid,
        spread_bps: 2.0,
        bid_depth_btc_10bps: 5.0,
        ask_depth_btc_10bps: 5.0,
        bid_depth_usd_10bps: 500_000.0,
        ask_depth_usd_10bps: 500_000.0,
        imbalance_10bps: 0.0,
    }
}
