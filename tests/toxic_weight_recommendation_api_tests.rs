...
#[tokio::test]
async fn toxic_weight_recommendation_api_returns_analysis_only_payloads() {
    ...
    let status = test_http_get(format!(
        "http://{addr}/api/toxicity/weight-recommendation/status"
    ))
    .await
    ...
    let summary = test_http_get(format!(
        "http://{addr}/api/toxicity/weight-recommendation/summary"
    ))
    .await
    ...
    assert_eq!(summary_payload["runtimeModified"], false);
    assert_eq!(summary_payload["runtimeWeightModified"], false);
    assert_eq!(summary_payload["configModified"], false);
    assert!(summary_payload["recommendations"].is_array());
    assert!(summary_payload["bySignalType"].is_array());
    assert!(summary_payload["bySymbol"].is_array());
    assert!(summary_payload["reviewFlags"].is_array());

    let by_symbol = test_http_get(format!(
        "http://{addr}/api/toxicity/weight-recommendation/{}",
        "BTC-PERP"
    ))
    .await
    ...
    assert_eq!(by_symbol_payload["analysisOnly"], true);
