...
#[tokio::test]
async fn whale_flow_candidate_history_api_exposes_read_only_history_visibility() {
    ...
    *state.shared_flow_for_tests().write() = sample_flow_state();
    ...
    let empty_recent = test_http_get(format!(
        "http://{addr}/api/toxicity/whale-flow/history/recent"
    ))
    .await
    .expect("empty recent response");
    let empty_recent_payload: serde_json::Value =
        empty_recent.json().await.expect("empty recent json");
    assert_eq!(empty_recent_payload["status"], "empty_candidate_history");
    assert_eq!(
        empty_recent_payload["operatorNotes"][0],
        "No whale candidates have been recorded in bounded memory yet."
    );

    let whale_recent = test_http_get(format!("http://{addr}/api/toxicity/whale-flow/recent"))
        .await
        .expect("whale recent response");
    assert_eq!(whale_recent.status(), reqwest::StatusCode::OK);

    let status = test_http_get(format!(
        "http://{addr}/api/toxicity/whale-flow/history/status"
    ))
    .await
    ...
    assert_eq!(status_payload["currentCandidates"], 1);
    assert_eq!(status_payload["maxCandidates"], 500);
    assert_eq!(status_payload["recordedCount"], 1);
    assert_eq!(status_payload["deduplicatedCount"], 0);
    assert_eq!(status_payload["evictedCount"], 0);
    assert_eq!(status_payload["resolvedMarkoutEvidenceCount"], 0);
    assert_eq!(status_payload["calibrationReady"], false);

    let recent = test_http_get(format!(
        "http://{addr}/api/toxicity/whale-flow/history/recent"
    ))
    .await
    ...
    assert_eq!(recent_payload["items"][0]["symbol"], "BTC-PERP");
    assert_eq!(
        recent_payload["items"][0]["classification"],
        "aggressive_buy"
    );
    assert_eq!(recent_payload["items"][0]["outcomeStatus"], "unresolved");

    let by_symbol = test_http_get(format!(
        "http://{addr}/api/toxicity/whale-flow/history/BTC-PERP"
    ))
    .await
    ...
    assert_eq!(by_symbol_payload["status"], "candidate_history_available");
