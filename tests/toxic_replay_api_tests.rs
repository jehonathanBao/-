...
#[tokio::test]
async fn toxic_replay_api_returns_analysis_only_payloads() {
    ...
    let status = test_http_get(format!("http://{addr}/api/toxicity/replay/status"))
        .await
    ...
    let recent = test_http_get(format!("http://{addr}/api/toxicity/replay/recent"))
        .await
    ...
    let by_symbol = test_http_get(format!("http://{addr}/api/toxicity/replay/{}", "BTC-PERP"))
        .await
    ...
    let latest = test_http_get(format!(
        "http://{addr}/api/toxicity/replay/{}/latest",
        "BTC-PERP"
    ))
    .await
    ...
    assert_eq!(latest_payload["runtimeModified"], false);
    assert_eq!(latest_payload["mode"], "analysis_only");

    let by_signal = test_http_get(format!(
        "http://{addr}/api/toxicity/replay/{}/{}",
        "BTC-PERP", "missing-signal"
    ))
    .await
