...
use btc_toxic_flow_monitor_rs::{
    calibration::{
        ...
        manual_signoff_store::{
            ManualSignoffDecision, ManualSignoffGateStatus, ManualSignoffInput, ManualSignoffStore,
        },
...
#[test]
fn approved_signoff_records_and_reports_signed_off() {
    let report_dir = temp_report_dir("manual_signoff_store_approved");
    write_ready_calibration_report(&report_dir, "calibration-9100", 9_100);
    approve_all_latest_recommendations(&report_dir, 9_110);
    create_export(&report_dir);

    let store = ManualSignoffStore::new(report_dir.clone(), test_config(report_dir.clone()));
    ...
    assert_eq!(response.status, ManualSignoffGateStatus::SignedOff);
    assert!(response.read_only);

    let status = store.status().expect("status");
    assert_eq!(status.status, ManualSignoffGateStatus::SignedOff);
    assert_eq!(
        status.latest_signoff.expect("latest signoff").operator,
        "byhdo"
    );
}

#[test]
fn rejected_signoff_is_recorded() {
    let report_dir = temp_report_dir("manual_signoff_store_rejected");
    write_ready_calibration_report(&report_dir, "calibration-9110", 9_110);
    approve_all_latest_recommendations(&report_dir, 9_120);
    create_export(&report_dir);
    ...
    store
        .create_signoff_at(
            ManualSignoffInput {
                operator: "byhdo".to_string(),
                decision: ManualSignoffDecision::Rejected,
                note: Some("hold".to_string()),
            },
            now_ms,
        )
        .expect("create rejected signoff");

    let status = store.status().expect("status");
    assert_eq!(status.status, ManualSignoffGateStatus::Rejected);
    assert!(status.read_only);
}

#[test]
fn evidence_change_marks_signoff_stale() {
    let report_dir = temp_report_dir("manual_signoff_store_stale");
    write_ready_calibration_report(&report_dir, "calibration-9120", 9_120);
    approve_all_latest_recommendations(&report_dir, 9_130);
    create_export(&report_dir);
    ...
    approve_all_latest_recommendations(&report_dir, 9_141);

    let status = store.status().expect("status");
    assert_eq!(status.status, ManualSignoffGateStatus::SignoffStale);
}

#[test]
fn old_signoff_expires() {
    let report_dir = temp_report_dir("manual_signoff_store_expired");
    write_ready_calibration_report(&report_dir, "calibration-9130", 9_130);
    approve_all_latest_recommendations(&report_dir, 9_140);
    create_export(&report_dir);

    let store = ManualSignoffStore::new(report_dir.clone(), test_config(report_dir.clone()));
    let old_ms = chrono::Local::now().timestamp_millis() - (25 * 60 * 60 * 1000);
    store
        .create_signoff_at(
            ManualSignoffInput {
                operator: "byhdo".to_string(),
        ...
        .expect("create old signoff");

    let status = store.status().expect("status");
    assert_eq!(status.status, ManualSignoffGateStatus::SignoffExpired);
}

#[test]
fn readiness_not_ready_blocks_approved_signoff() {
    let report_dir = temp_report_dir("manual_signoff_store_blocked");
    let store = ManualSignoffStore::new(report_dir.clone(), test_config(report_dir.clone()));
    let err = store
        .create_signoff_at(
            ManualSignoffInput {
                operator: "byhdo".to_string(),
                decision: ManualSignoffDecision::Approved,
                note: Some("no report".to_string()),
            },
            9_150,
        )
        .expect_err("approved signoff should fail");

    assert_eq!(err.to_string(), "readiness_not_ready");
}
...
fn create_export(report_dir: &PathBuf) {
    let _ = ManualParameterExportStore::new(report_dir)
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("signoff export".to_string()),
        })
