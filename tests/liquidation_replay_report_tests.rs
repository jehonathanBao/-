...
#[test]
fn replay_report_includes_liquidation_summary_and_evidence() {
    ...
    assert!(markdown.contains("## Liquidation Cluster Summary"));
    assert!(markdown.contains("## Liquidation Evidence Events"));
    assert!(markdown.contains("liq_cluster_nearby"));
    assert!(markdown.contains("possible_liq_hunt_setup"));
}

#[test]
fn replay_report_written_file_contains_liquidation_evidence() {
    let fixture = fixture_path();
    let mut runner = ReplayRunner::new(test_config());
    let report = runner
        .run_file(fixture.to_str().expect("utf8 path"))
    ...
    let dir = temp_dir("liquidation_replay_report");
...
fn fixture_path() -> std::path::PathBuf {
    let path = temp_dir("liquidation_fixture").join("sample-liquidation-hunt.jsonl");
