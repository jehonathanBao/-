...
#[tokio::test]
async fn toxic_signal_report_api_returns_read_only_daily_digest() {
    ...
    let status = test_http_get(format!("http://{addr}/api/toxicity/signal-report/status"))
        .await
    ...
    let daily = test_http_get(format!("http://{addr}/api/toxicity/signal-report/daily"))
        .await
        .expect("daily response");
    assert_eq!(daily.status(), reqwest::StatusCode::OK);
    let daily_payload: serde_json::Value = daily.json().await.expect("daily json");
    assert_eq!(daily_payload["readOnly"], true);
    assert_eq!(daily_payload["runtimeModified"], false);
    assert_eq!(daily_payload["analysisOnly"], true);
    assert_eq!(daily_payload["executionEnabled"], false);
    assert_eq!(daily_payload["filter"]["viewOnly"], true);
    assert_eq!(daily_payload["filter"]["persistentWatchlistEnabled"], false);
    assert_eq!(daily_payload["filter"]["runtimeMonitorModified"], false);
    assert!(daily_payload["summary"].is_object());
    assert!(daily_payload["markdown"].as_str().is_some());

    let filtered = test_http_get(format!(
        "http://{addr}/api/toxicity/signal-report/daily?symbol=%20btc-perp%20"
    ))
    .await
    ...
    assert_eq!(filtered_payload["filter"]["viewOnly"], true);

    let rolling = test_http_get(format!("http://{addr}/api/toxicity/signal-report/rolling"))
        .await
        .expect("rolling response");
    assert_eq!(rolling.status(), reqwest::StatusCode::OK);
    let rolling_payload: serde_json::Value = rolling.json().await.expect("rolling json");
    assert_eq!(rolling_payload["readOnly"], true);
    assert_eq!(rolling_payload["runtimeModified"], false);
    assert_eq!(rolling_payload["analysisOnly"], true);
    assert_eq!(rolling_payload["executionEnabled"], false);
    assert_eq!(rolling_payload["retentionMode"], "in_memory_bounded");
    assert_eq!(rolling_payload["durableStorageEnabled"], false);
    assert_eq!(rolling_payload["databaseWriteEnabled"], false);
    assert_eq!(rolling_payload["window"], "7d");
    assert!(rolling_payload["summary"].is_object());
    assert!(rolling_payload["markdown"].as_str().is_some());
