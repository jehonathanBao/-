...
#[tokio::test]
async fn manual_apply_runbook_api_serves_json_and_markdown() {
    let report_dir = temp_report_dir("manual_apply_runbook_api");
    write_calibration_report(&report_dir, "calibration-8600", 8_600);
    ...
    let create = export_store
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("runbook api test".to_string()),
        })
    ...
    let latest = test_http_get(format!(
        "http://{addr}/api/parameter-review/exports/latest/runbook"
    ))
    .await
    .expect("latest runbook")
    .json::<serde_json::Value>()
    .await
    .expect("latest runbook json");
    ...
    let markdown = test_http_get(format!(
        "http://{addr}/api/parameter-review/exports/{export_id}/runbook.md"
    ))
    .await
    ...
    assert!(markdown_text.contains("# Manual Apply Runbook"));

    server.abort();
}

#[tokio::test]
async fn manual_apply_runbook_api_reports_missing_exports_cleanly() {
    let report_dir = temp_report_dir("manual_apply_runbook_api_empty");
    write_calibration_report(&report_dir, "calibration-8700", 8_700);
    ...
    let missing = test_http_get(format!(
        "http://{addr}/api/parameter-review/exports/does-not-exist/runbook.md"
    ))
    .await
    .expect("missing markdown");
    assert_eq!(missing.status(), reqwest::StatusCode::NOT_FOUND);
    let text = missing.text().await.expect("missing markdown text");
