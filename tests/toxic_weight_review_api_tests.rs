...
#[tokio::test]
async fn toxic_weight_review_api_returns_export_only_analysis_payloads() {
    ...
    let status = test_http_get(format!("http://{addr}/api/toxicity/weight-review/status"))
        .await
    ...
    assert_eq!(status_payload["exportOnly"], true);
    ...
    let summary = test_http_get(format!("http://{addr}/api/toxicity/weight-review/latest"))
        .await
    ...
    assert_eq!(summary_payload["exportOnly"], true);
    assert!(summary_payload["reviewItems"].is_array());
    assert!(summary_payload["governanceNotes"].is_array());
    assert!(summary_payload["bySymbol"].is_array());

    let by_symbol = test_http_get(format!(
        "http://{addr}/api/toxicity/weight-review/{}",
        "BTC-PERP"
    ))
    .await
    ...
    assert_eq!(by_symbol_payload["exportOnly"], true);

    let export = test_http_get(format!(
        "http://{addr}/api/toxicity/weight-review/{}/export",
        "BTC-PERP"
    ))
    .await
    ...
    assert_eq!(export_payload["exportOnly"], true);
    assert_eq!(export_payload["autoApplyEnabled"], false);
    assert!(export_payload["manualReviewChecklist"].is_array());
    assert!(export_payload["doNotApplyConditions"].is_array());
    assert!(export_payload["rollbackNotes"].is_array());
    assert!(export_payload["evidenceSources"].is_array());
    assert_eq!(
        export_payload["recommendationSummary"]["symbol"],
        "BTC-PERP"
    );
    assert!(export_payload["governanceNotesMarkdown"]
        .as_str()
        .unwrap_or("")
