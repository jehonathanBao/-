...
use btc_toxic_flow_monitor_rs::{
    ...
    types::{liq_hunt::LiqHuntSignalLevel, market::Venue, toxic::ToxicSeverity},
};

#[tokio::test]
async fn liq_hunt_state_api_returns_none_level_without_data() {
    let state = AppState::new(test_config());
    state
        .liq_hunt_service_for_tests()
        .compute_once_for_tests(1_770_000_000_000);
    ...
    let response = test_http_get(format!("http://{addr}/api/liq-hunt-state"))
        .await
        .expect("response");
    assert_eq!(response.status(), reqwest::StatusCode::OK);
    let payload: serde_json::Value = response.json().await.expect("json");
    assert_eq!(payload["symbol"], "BTC-PERP");
    assert_eq!(payload["result"]["level"], "none");

    server.abort();
}

#[test]
fn status_response_includes_liq_hunt_summary() {
    let state = AppState::new(test_config());
    state
        .liq_hunt_service_for_tests()
        .compute_once_for_tests(1_770_000_000_000);
    ...
    assert!(status.liq_hunt.enabled);
    assert_eq!(status.liq_hunt.level, LiqHuntSignalLevel::None);
    assert_eq!(status.liq_hunt.score, 0.0);
}
