...
#[tokio::test]
async fn toxic_governance_proposal_api_returns_proposal_only_payloads() {
    ...
    let status = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-proposal/status"
    ))
    .await
    ...
    let summary = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-proposal/summary"
    ))
    .await
    ...
    let by_symbol = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-proposal/{}",
        "BTC-PERP"
    ))
    .await
    ...
    assert_eq!(by_symbol_payload["proposalOnly"], true);

    let export = test_http_get(format!(
        "http://{addr}/api/toxicity/governance-proposal/export"
    ))
    .await
