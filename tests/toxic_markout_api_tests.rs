...
#[tokio::test]
async fn toxic_markout_api_returns_analysis_only_payloads() {
    ...
    let status = test_http_get(format!("http://{addr}/api/toxicity/markout/status"))
        .await
    ...
    let recent = test_http_get(format!("http://{addr}/api/toxicity/markout/recent"))
        .await
    ...
    let by_symbol = test_http_get(format!("http://{addr}/api/toxicity/markout/{}", "BTC-PERP"))
        .await
    ...
    let by_signal = test_http_get(format!(
        "http://{addr}/api/toxicity/markout/signal/{}?symbol=BTC-PERP",
        "missing-signal"
    ))
    .await
    .expect("signal response");
    assert_eq!(by_signal.status(), reqwest::StatusCode::OK);
    let by_signal_payload: serde_json::Value = by_signal.json().await.expect("signal json");
    assert_eq!(by_signal_payload["readOnly"], true);
    assert_eq!(by_signal_payload["available"], false);
