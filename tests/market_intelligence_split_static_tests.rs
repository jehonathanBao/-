...
fn btc_engine_does_not_expose_altcoin_manipulation_score() {
    ...
    assert!(!btc_engine.contains("manipulation_score"));
...
#[test]
fn fusion_core_is_domain_aware_without_altcoin_control_logic() {
    ...
    assert!(!fusion_core.contains("AltcoinManipulation"));
    assert!(!fusion_core.contains("altcoin_control_score"));
