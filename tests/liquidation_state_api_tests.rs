...
#[tokio::test]
async fn liquidation_state_api_returns_empty_state() {
    let state = AppState::new(test_config());
    state
        .liquidation_service_for_tests()
        .compute_once_for_tests(1_760_000_000_000);
    ...
    let response = test_http_get(format!("http://{addr}/api/liquidation-state"))
        .await
        .expect("response");
...
#[test]
fn status_response_includes_liquidation_summary() {
    let state = AppState::new(test_config());
    state
        .liquidation_service_for_tests()
        .compute_once_for_tests(1_760_000_000_000);
    let status = build_status_response(&state);
    assert!(status.liquidation.enabled);
    assert!(!status.liquidation.liq_cluster_nearby);
}
