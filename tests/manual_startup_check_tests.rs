...
#[test]
fn missing_report_returns_missing_report_status() {
    let report_dir = temp_report_dir("manual_startup_missing_report");
    let store = ManualStartupCheckStore::new(&report_dir, test_config(report_dir.clone()));
    let response = store.run_check().expect("startup check");

    assert!(!response.ok);
    assert_eq!(response.status, ManualStartupStatus::MissingReport);
    assert!(response.read_only);
}

#[test]
fn missing_patch_returns_needs_review_status() {
    let report_dir = temp_report_dir("manual_startup_missing_patch");
    write_calibration_report(&report_dir, "calibration-9000", 9_000);

    let store = ManualStartupCheckStore::new(&report_dir, test_config(report_dir.clone()));
    let response = store.run_check().expect("startup check");

    assert!(!response.ok);
    assert_eq!(response.status, ManualStartupStatus::NeedsReview);
    assert!(response
        .checks
        .iter()
        .any(|check| check.name == "manual_patch_export" && !check.ok));
}

#[test]
fn failed_dry_run_returns_blocked_status() {
    let report_dir = temp_report_dir("manual_startup_blocked");
    write_calibration_report(&report_dir, "calibration-9010", 9_010);
    approve_all_latest_recommendations(&report_dir, 9_020);
    ...
    assert!(!response.ok);
    assert_eq!(response.status, ManualStartupStatus::Blocked);
    assert!(response
        .checks
        .iter()
...
#[test]
fn fully_reviewed_and_passing_chain_returns_ready_status() {
    let report_dir = temp_report_dir("manual_startup_ready");
    write_ready_calibration_report(&report_dir, "calibration-9020", 9_020);
    approve_all_latest_recommendations(&report_dir, 9_030);
    ...
    assert!(response.ok);
    assert_eq!(response.status, ManualStartupStatus::ReadyForManualApply);
    assert!(response.read_only);
    assert!(response
        .checks
        .iter()
...
fn create_export(report_dir: &PathBuf) -> String {
    ManualParameterExportStore::new(report_dir)
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("startup check test".to_string()),
        })
