...
#[tokio::test]
async fn manual_governance_index_api_serves_json() {
    let report_dir = temp_report_dir("manual_governance_api_ready");
    write_ready_calibration_report(&report_dir, "calibration-9920", 9_920);
    approve_all_latest_recommendations(&report_dir, 9_930);
    ...
    let payload = test_http_get(format!(
        "http://{addr}/api/calibration/manual-governance/index"
    ))
    .await
    ...
    assert_eq!(payload["auditStoryStatus"], "AVAILABLE");
    assert_eq!(payload["currentBlocker"], serde_json::Value::Null);
...
#[tokio::test]
async fn manual_governance_index_api_surfaces_blocking_reasons() {
    let report_dir = temp_report_dir("manual_governance_api_blocked");
    write_ready_calibration_report(&report_dir, "calibration-9930", 9_930);
    approve_all_latest_recommendations(&report_dir, 9_940);
    create_export(&report_dir);
    ...
    assert_eq!(payload["currentBlocker"], "SIGNOFF_MISSING");
