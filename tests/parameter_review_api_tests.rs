...
#[tokio::test]
async fn parameter_review_api_lists_latest_and_persists_review_ledger() {
    let report_dir = temp_report_dir("parameter_review_api");
    write_calibration_report(&report_dir, "calibration-3000", 3_000);
    ...
    let latest = test_http_get(format!(
        "http://{addr}/api/parameter-review/recommendations/latest"
    ))
    .await
    ...
    .await
    .expect("latest json");
    let recommendation_id = latest["recommendations"][0]["recommendationId"]
        .as_str()
    ...
    let response = client
        .post(format!("http://{addr}/api/parameter-review/reviews"))
        .json(&serde_json::json!({
            "recommendation_id": recommendation_id,
            "report_id": "calibration-3000",
            "status": "watch",
            "reviewer_note": "observe more replay cases",
            "reviewer": "codex"
        }))
        .send()
        .await
    ...
    let reviews = test_http_get(format!("http://{addr}/api/parameter-review/reviews"))
        .await
        .expect("reviews response")
        .json::<serde_json::Value>()
        .await
        .expect("reviews json");
    assert_eq!(reviews["reviews"][0]["status"], "watch");
    assert_eq!(reviews["reviews"][0]["reviewer"], "codex");
