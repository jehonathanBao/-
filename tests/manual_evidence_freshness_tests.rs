...
#[test]
fn no_signoff_returns_no_signoff() {
    let report_dir = temp_report_dir("manual_evidence_freshness_no_signoff");
    write_ready_calibration_report(&report_dir, "calibration-9300", 9_300);
    let store =
        ManualEvidenceFreshnessStore::new(report_dir.clone(), test_config(report_dir.clone()));
    let response = store.freshness().expect("freshness");
    assert_eq!(response.status, ManualEvidenceFreshnessStatus::NoSignoff);
    assert!(response.read_only);
}

#[test]
fn matching_signoff_returns_fresh() {
    let report_dir = temp_report_dir("manual_evidence_freshness_fresh");
    write_ready_calibration_report(&report_dir, "calibration-9310", 9_310);
    approve_all_latest_recommendations(&report_dir, 9_320);
    ...
    let store =
        ManualEvidenceFreshnessStore::new(report_dir.clone(), test_config(report_dir.clone()));
    let response = store.freshness().expect("freshness");
    assert_eq!(response.status, ManualEvidenceFreshnessStatus::Fresh);
    assert!(response.age_ms.is_some());
    assert!(response.expires_in_ms.is_some());
}

#[test]
fn changed_review_marks_stale() {
    let report_dir = temp_report_dir("manual_evidence_freshness_stale");
    write_ready_calibration_report(&report_dir, "calibration-9320", 9_320);
    approve_all_latest_recommendations(&report_dir, 9_330);
    create_export(&report_dir);
    create_approved_signoff(&report_dir, chrono::Local::now().timestamp_millis());
    approve_all_latest_recommendations(&report_dir, 9_331);

    let store =
        ManualEvidenceFreshnessStore::new(report_dir.clone(), test_config(report_dir.clone()));
    let response = store.freshness().expect("freshness");
    assert_eq!(response.status, ManualEvidenceFreshnessStatus::Stale);
    assert!(response
        .changed_evidence
        .iter()
        .any(|name| name == "recommendation_review"));
}

#[test]
fn old_signoff_marks_expired() {
    let report_dir = temp_report_dir("manual_evidence_freshness_expired");
    write_ready_calibration_report(&report_dir, "calibration-9330", 9_330);
    approve_all_latest_recommendations(&report_dir, 9_340);
    create_export(&report_dir);
    let old_ms = chrono::Local::now().timestamp_millis() - (31 * 60 * 1000);
    create_approved_signoff(&report_dir, old_ms);

    let store =
        ManualEvidenceFreshnessStore::new(report_dir.clone(), test_config(report_dir.clone()));
    let response = store.freshness().expect("freshness");
    assert_eq!(response.status, ManualEvidenceFreshnessStatus::Expired);
    assert_eq!(response.ttl_ms, 30 * 60 * 1000);
    assert!(response.age_ms.unwrap_or_default() > response.ttl_ms);
}

#[test]
fn missing_export_marks_missing_evidence() {
    let report_dir = temp_report_dir("manual_evidence_freshness_missing");
    write_ready_calibration_report(&report_dir, "calibration-9340", 9_340);
    approve_all_latest_recommendations(&report_dir, 9_350);
    create_export(&report_dir);
    create_approved_signoff(&report_dir, chrono::Local::now().timestamp_millis());

    let export_dir = report_dir.parent().expect("runtime dir").join("exports");
    let latest = ManualParameterExportStore::new(&report_dir)
        .latest_export()
        .expect("latest export")
        .expect("export entry");
    fs::remove_file(export_dir.join(format!("{}.json", latest.summary.export_id)))
        .expect("remove export json");
...
#[test]
fn read_only_false_marks_readiness_not_ready() {
    let report_dir = temp_report_dir("manual_evidence_freshness_readiness");
    write_ready_calibration_report(&report_dir, "calibration-9350", 9_350);
    approve_all_latest_recommendations(&report_dir, 9_360);
    create_export(&report_dir);
    create_approved_signoff(&report_dir, chrono::Local::now().timestamp_millis());

    let mut config = test_config(report_dir.clone());
    config.read_only = false;
    let store = ManualEvidenceFreshnessStore::new(report_dir, config);
    let response = store.freshness().expect("freshness");
    assert_eq!(
        response.status,
...
fn create_export(report_dir: &Path) {
    let _ = ManualParameterExportStore::new(report_dir)
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("freshness export".to_string()),
        })
