...
#[test]
fn latest_runbook_generates_checklists_and_manual_only_flags() {
    let report_dir = temp_report_dir("manual_apply_runbook");
    write_calibration_report(&report_dir, "calibration-8400", 8_400);
    approve_recommendations(
        &report_dir,
        &["toxicity.threshold_btc", "toxicity.min_toxic_ratio"],
    );
    create_export(&report_dir);

    let store = ManualApplyRunbookStore::new(&report_dir, Some(test_config(report_dir.clone())));
    let runbook = store
        .latest_runbook()
        .expect("latest runbook")
        .expect("runbook");

    assert!(runbook.ok);
    assert_eq!(runbook.apply_mode, "manual_only");
    assert!(!runbook.runtime_modified);
    assert_eq!(runbook.summary.total_patch_fields, 2);
    assert_eq!(runbook.summary.changed_fields, 1);
    assert_eq!(runbook.summary.missing_in_current_config, 1);
    assert_eq!(runbook.summary.risk_level, "review_required");
    assert!(runbook
        .pre_apply_checklist
        .iter()
        .any(|item| item.id == "resolve-missing-fields"));
    assert!(!runbook.safety_guards.auto_apply_enabled);
    assert!(!runbook.safety_guards.runtime_reload_enabled);
    assert!(!runbook.safety_guards.calibration_runner_triggered);
    assert!(!runbook.safety_guards.trading_path_touched);

    let markdown = store
        .latest_markdown()
        .expect("latest markdown")
        .expect("markdown");
    assert!(markdown.contains("# Manual Apply Runbook"));
    assert!(markdown.contains("## Post-apply Verification"));
    assert!(markdown.contains("## Rollback Plan"));
}

#[test]
fn runbook_store_returns_none_or_config_unavailable_when_expected() {
    let report_dir = temp_report_dir("manual_apply_runbook_empty");
    write_calibration_report(&report_dir, "calibration-8500", 8_500);

    let store = ManualApplyRunbookStore::new(&report_dir, Some(test_config(report_dir.clone())));
    assert!(store.latest_runbook().expect("latest").is_none());

    approve_recommendations(&report_dir, &["toxicity.threshold_btc"]);
    create_export(&report_dir);

    let no_config_store = ManualApplyRunbookStore::new(&report_dir, None);
    let error = no_config_store
        .latest_runbook()
        .expect_err("missing config should error");
    assert_eq!(error.to_string(), "current_config_unavailable");
}

fn approve_recommendations(report_dir: &PathBuf, keys: &[&str]) {
    ...
    for card in cards
        .iter()
        .filter(|card| keys.contains(&card.parameter_key.as_str()))
    {
        review_store
            .append_review(
                ReviewDecisionInput {
...
fn create_export(report_dir: &PathBuf) {
    ManualParameterExportStore::new(report_dir)
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("runbook test".to_string()),
        })
