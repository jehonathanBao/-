...
#[tokio::test]
async fn toxic_signal_fusion_api_returns_analysis_only_payloads() {
    ...
    let status = test_http_get(format!("http://{addr}/api/toxicity/fusion/status"))
        .await
    ...
    let recent = test_http_get(format!("http://{addr}/api/toxicity/fusion/recent"))
        .await
    ...
    let by_symbol = test_http_get(format!("http://{addr}/api/toxicity/fusion/{}", "BTC-PERP"))
        .await
