...
#[tokio::test]
async fn toxic_signal_health_api_returns_diagnostic_only_contract() {
    ...
    let status = test_http_get(format!("http://{addr}/api/toxicity/signal-health/status"))
        .await
    ...
    assert_eq!(status_payload["healthMode"], "diagnostic_only");
    assert_eq!(status_payload["repairEnabled"], false);
    assert_eq!(status_payload["backfillEnabled"], false);
    assert_eq!(status_payload["runtimeMutationEnabled"], false);

    let summary = test_http_get(format!("http://{addr}/api/toxicity/signal-health/summary"))
        .await
    ...
    assert_eq!(summary_payload["healthMode"], "diagnostic_only");
    assert_eq!(summary_payload["repairEnabled"], false);
    assert_eq!(summary_payload["backfillEnabled"], false);
    assert_eq!(summary_payload["runtimeMutationEnabled"], false);
    assert!(summary_payload["summary"].is_object());
    assert!(summary_payload["issues"].is_array());
    assert!(summary_payload["operatorNotes"].is_array());

    let by_symbol = test_http_get(format!("http://{addr}/api/toxicity/signal-health/BTC-PERP"))
        .await
        .expect("symbol response");
    assert_eq!(by_symbol.status(), reqwest::StatusCode::OK);
    let symbol_payload: serde_json::Value = by_symbol.json().await.expect("symbol json");
    assert_eq!(symbol_payload["selectedSymbol"], "BTC-PERP");
    assert_eq!(symbol_payload["readOnly"], true);
    assert_eq!(symbol_payload["executionEnabled"], false);
