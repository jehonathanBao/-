...
#[tokio::test]
async fn liquidation_toxicity_api_returns_analysis_only_status_and_recent_payload() {
    ...
    let status = test_http_get(format!("http://{addr}/api/toxicity/liquidation/status"))
        .await
    ...
    let recent = test_http_get(format!("http://{addr}/api/toxicity/liquidation/recent"))
        .await
    ...
    assert!(recent_payload["signals"]
        .as_array()
        .is_some_and(|items| items.iter().all(|signal| signal["readOnly"] == true)));
