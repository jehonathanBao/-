...
#[tokio::test]
async fn parameter_patch_diff_api_reads_latest_and_by_id() {
    let report_dir = temp_report_dir("parameter_patch_diff_api");
    write_calibration_report(&report_dir, "calibration-8200", 8_200);
    ...
    let create = export_store
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("api diff test".to_string()),
        })
        .expect("create export");
    let export_id = create.export_id.expect("export id");
    ...
    let latest = test_http_get(format!(
        "http://{addr}/api/parameter-review/exports/latest/diff"
    ))
    .await
    .expect("latest diff")
    .json::<serde_json::Value>()
    .await
    .expect("latest diff json");
    assert_eq!(latest["applyMode"], "manual_only");
    assert_eq!(latest["runtimeModified"], false);
    assert_eq!(latest["diff"]["exportId"], export_id);

    let by_id = test_http_get(format!(
        "http://{addr}/api/parameter-review/exports/{export_id}/audit"
    ))
    .await
    .expect("audit by id")
    .json::<serde_json::Value>()
    .await
    .expect("audit json");
    assert_eq!(by_id["applyMode"], "manual_only");
    assert_eq!(by_id["runtimeModified"], false);
    assert_eq!(by_id["audit"]["exportId"], export_id);

    server.abort();
}

#[tokio::test]
async fn parameter_patch_diff_api_returns_expected_empty_and_not_found_reasons() {
    let report_dir = temp_report_dir("parameter_patch_diff_api_empty");
    write_calibration_report(&report_dir, "calibration-8300", 8_300);
    ...
    let missing = test_http_get(format!(
        "http://{addr}/api/parameter-review/exports/does-not-exist/diff"
    ))
    .await
    .expect("missing diff");
    ...
    let missing_json = missing
        .json::<serde_json::Value>()
        .await
        .expect("missing diff json");
    assert_eq!(missing_json["reason"], "export_not_found");
    assert_eq!(missing_json["applyMode"], "manual_only");
