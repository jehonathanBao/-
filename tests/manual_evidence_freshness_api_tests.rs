...
#[tokio::test]
async fn freshness_api_returns_fresh_with_age_and_expiry() {
    let report_dir = temp_report_dir("manual_evidence_freshness_api_fresh");
    write_ready_calibration_report(&report_dir, "calibration-9400", 9_400);
    approve_all_latest_recommendations(&report_dir, 9_410);
    ...
    let response = test_http_get(format!(
        "http://{addr}/api/calibration/manual-evidence/freshness"
    ))
    .await
    ...
    assert!(response["ageMs"].as_i64().is_some());
    assert!(response["expiresInMs"].as_i64().is_some());

    server.abort();
}

#[tokio::test]
async fn freshness_api_surfaces_stale_changed_evidence() {
    let report_dir = temp_report_dir("manual_evidence_freshness_api_stale");
    write_ready_calibration_report(&report_dir, "calibration-9410", 9_410);
    approve_all_latest_recommendations(&report_dir, 9_420);
    create_export(&report_dir);
    create_approved_signoff(&report_dir, chrono::Local::now().timestamp_millis());
    approve_all_latest_recommendations(&report_dir, 9_421);
    ...
    assert!(response["changedEvidence"]
        .as_array()
        .expect("changed evidence")
        .iter()
        .any(|item| item == "recommendation_review"));

    server.abort();
}

#[tokio::test]
async fn freshness_api_reports_expired_status() {
    let report_dir = temp_report_dir("manual_evidence_freshness_api_expired");
    write_ready_calibration_report(&report_dir, "calibration-9420", 9_420);
    approve_all_latest_recommendations(&report_dir, 9_430);
    create_export(&report_dir);
    ...
    assert_eq!(response["ttlMs"], 30 * 60 * 1000);
    assert!(
        response["ageMs"].as_i64().unwrap_or_default()
            > response["ttlMs"].as_i64().unwrap_or_default()
    );
...
fn create_export(report_dir: &Path) {
    let _ = ManualParameterExportStore::new(report_dir)
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("freshness api export".to_string()),
        })
