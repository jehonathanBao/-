...
#[test]
fn latest_diff_classifies_changed_and_missing_fields() {
    let report_dir = temp_report_dir("parameter_patch_diff");
    write_calibration_report(&report_dir, "calibration-8000", 8_000);
    ...
    let export_response = export_store
        .create_export(ManualParameterExportRequest {
            ...
            note: Some("diff test".to_string()),
        })
        .expect("create export");
    assert!(export_response.export_created);

    let diff_store =
        ParameterPatchDiffStore::new(&report_dir, Some(test_config(report_dir.clone())));
    let diff = diff_store
        .latest_diff()
        .expect("latest diff")
        .expect("diff payload");

    assert_eq!(diff.apply_mode, "manual_only");
    assert!(!diff.runtime_modified);
    assert_eq!(diff.summary.approved_recommendations_count, 2);

    let threshold = diff
        .entries
        .iter()
        .find(|entry| entry.key == "toxicity.threshold_btc")
        .expect("threshold diff");
    assert_eq!(threshold.change_type, ParameterPatchChangeType::Changed);
    assert_eq!(threshold.numeric_delta, Some(200.0));
    assert_eq!(threshold.percent_delta, Some(0.2));

    let ratio = diff
        .entries
        .iter()
        .find(|entry| entry.key == "toxicity.min_toxic_ratio")
        .expect("ratio diff");
    assert_eq!(
        ratio.change_type,
...
#[test]
fn diff_store_handles_missing_exports_and_missing_current_config() {
    let report_dir = temp_report_dir("parameter_patch_diff_empty");
    write_calibration_report(&report_dir, "calibration-8100", 8_100);

    let diff_store =
        ParameterPatchDiffStore::new(&report_dir, Some(test_config(report_dir.clone())));
    assert!(diff_store.latest_diff().expect("latest diff").is_none());
    ...
    let no_config_store = ParameterPatchDiffStore::new(&report_dir, None);
    let error = no_config_store
        .latest_diff()
        .expect_err("missing config error");
