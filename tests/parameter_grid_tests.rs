use btc_toxic_flow_monitor_rs::{
    calibration::parameter_grid::CalibrationGrid,
    config::{
...
#[test]
fn default_grid_contains_expected_comparison_sets() {
    let grid = CalibrationGrid::default_for_config(&test_config());
    assert_eq!(grid.threshold_comparison.len(), 5);
    assert_eq!(grid.toxic_ratio_comparison.len(), 4);
    assert_eq!(grid.vpin_parameter_comparison.len(), 27);
    assert_eq!(grid.liq_hunt_score_comparison.len(), 4);
    assert_eq!(grid.baseline.label, "baseline");
}
