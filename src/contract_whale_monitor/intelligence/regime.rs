...
pub fn derive_market_regime(
    ...
    let liquidation_ratio = items
        .iter()
        .filter(|item| item.liquidation_suspected || item.liquidation_ratio.unwrap_or_default() >= 0.10)
        .count() as f64
        / items.len() as f64;

    let structure_hint = market_structure_lite.regime_type.to_ascii_lowercase();
    let (regime, confidence, reason) = if liquidation_ratio >= 0.45
        || market_structure_lite.extreme_impact_confirmed
    {
        (
            "LIQUIDATION_PHASE",
