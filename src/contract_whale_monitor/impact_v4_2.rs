...
use super::{
    ...
    impact_forecast::{
        build_forecast, evaluate_horizon_outcomes, ContractWhaleHorizonOutcome,
        ContractWhaleHorizonStats, ContractWhaleMultiHorizonImpactForecast,
        ContractWhaleOutcomeInputs, HORIZONS_SEC,
    },
    types::ContractWhaleSignal,
};

pub const CONTRACT_WHALE_IMPACT_FORECAST_V4_2_VERSION: &str = "cwm_impact_v4_2_hybrid";
...
pub fn effective_sample_size(weights: &[f64]) -> f64 {
    let sum = weights.iter().copied().filter(|v| v.is_finite() && *v > 0.0).sum::<f64>();
    let square_sum = weights
        ...
        .map(|v| v * v)
        .sum::<f64>();
    if square_sum <= 0.0 { 0.0 } else { (sum * sum / square_sum).max(0.0) }
}
...
fn valid_prior<'a>(signal: &ContractWhaleSignal, outcome: &'a ContractWhaleHorizonOutcome) -> bool {
    outcome.symbol.eq_ignore_ascii_case(&signal.symbol)
        && outcome.event_ts < signal.ts
        && outcome
            .event_ts
            .saturating_add(outcome.horizon_sec as i64 * 1_000)
            <= signal.ts
        && outcome.signed_markout_bps.is_some()
        && (outcome.outcome_version == super::impact_forecast::CONTRACT_WHALE_IMPACT_FORECAST_VERSION
            || outcome.outcome_version == CONTRACT_WHALE_IMPACT_FORECAST_V4_2_VERSION)
}
...
fn cohort_weight(signal: &ContractWhaleSignal, outcome: &ContractWhaleHorizonOutcome) -> f64 {
    ...
    if exact { return 1.0; }
    if outcome.behavior == behavior && outcome.direction == direction && outcome.market_regime == regime {
        return 0.82;
    }
    if outcome.behavior == behavior && outcome.direction == direction { return 0.68; }
    if outcome.behavior == behavior { return 0.55; }
    if outcome.direction == direction && outcome.market_regime == regime { return 0.48; }
    if outcome.direction == direction { return 0.38; }
    0.20
}
...
fn weighted_quantile(values: &[(f64, f64)], q: f64) -> Option<f64> {
    ...
    for (value, weight) in &values {
        cumulative += *weight;
        if cumulative >= target { return Some(*value); }
    }
    values.last().map(|(value, _)| *value)
}

fn model_estimate(signal: &ContractWhaleSignal, horizon_sec: u64) -> (f64, f64, f64, f64, f64, f64) {
    let direction = if direction_key(signal) == "bearish" { -1.0 } else { 1.0 };
    let horizon_factor = match horizon_sec { 900 => 0.70, 3_600 => 1.0, 14_400 => 1.35, _ => 1.80 };
    ...
    let base = (2.0 + signal.score as f64 * 0.22 + price_move + dominance + oi + liquidation) * horizon_factor;
    let quality = (signal.data_quality as f64 / 100.0).clamp(0.0, 1.0);
    let median = direction * base.max(1.0);
    let width = (base * (1.65 - quality * 0.65) + 4.0).max(4.0);
    let p25 = median - direction * width;
    let p75 = median + direction * width;
    let direction_probability = (0.50 + signal.score as f64 / 420.0 + quality * 0.06)
        .clamp(0.50, 0.92);
    ...
    (median, p25.min(p75), p25.max(p75), direction_probability, reversal_probability, structure_probability)
}
...
fn blended_horizon(
    ...
    let mut direction = 0;
    let mut global = 0;
    for outcome in outcomes.iter().filter(|outcome| outcome.horizon_sec == horizon_sec && valid_prior(signal, outcome)) {
        let weight = cohort_weight(signal, outcome);
        let markout = outcome.signed_markout_bps.unwrap_or(0.0);
        weighted.push((markout, weight));
        weights.push(weight);
        if outcome.follow_through.is_some() { follow.push((outcome.follow_through == Some(true), weight)); }
        if outcome.structure_break.is_some() { structure.push((outcome.structure_break == Some(true), weight)); }
        if let Some(value) = outcome.mfe_bps { mfe.push((value, weight)); }
        if let Some(value) = outcome.mae_bps { mae.push((value, weight)); }
        let behavior = behavior_key(signal);
        let dir = direction_key(signal);
        let regime = regime_key(signal);
        let intensity = intensity_key(signal);
        if outcome.behavior == behavior && outcome.direction == dir && outcome.market_regime == regime && outcome.intensity_bucket == intensity { exact += 1; }
        else if outcome.behavior == behavior && outcome.direction == dir && outcome.market_regime == regime { behavior_direction_regime += 1; }
        else if outcome.behavior == behavior && outcome.direction == dir { behavior_direction += 1; }
        else if outcome.direction == dir && outcome.market_regime == regime { direction_regime += 1; }
        else if outcome.direction == dir { direction += 1; }
        else { global += 1; }
    }
    let raw = weighted.len();
    let n_eff = effective_sample_size(&weights);
    let (model_median, model_p25, model_p75, model_direction, model_reversal, model_structure) = model_estimate(signal, horizon_sec);
    ...
    let sample_weight = if n_eff <= 0.0 { 0.0 } else { n_eff / (n_eff + prior_strength.max(1.0)) };
    let model_weight = 1.0 - sample_weight;
    let blend = |historical: Option<f64>, model: f64| historical.map(|v| v * sample_weight + model * model_weight).unwrap_or(model);
    ...
    let follow_rate = if follow.is_empty() { None } else { Some(follow.iter().map(|(v, w)| if *v { *w } else { 0.0 }).sum::<f64>() / follow.iter().map(|(_, w)| *w).sum::<f64>()) };
    let structure_rate = if structure.is_empty() { None } else { Some(structure.iter().map(|(v, w)| if *v { *w } else { 0.0 }).sum::<f64>() / structure.iter().map(|(_, w)| *w).sum::<f64>()) };
    let expected_low_price = None;
    let expected_high_price = None;
    let rating = match signal.score { score if score >= 85 && signal.data_quality >= 75 => "S", score if score >= 70 && signal.data_quality >= 70 => "A", score if score >= 55 && signal.data_quality >= 60 => "B", _ => "C" };
    let stats = ContractWhaleHorizonStats {
        horizon: match horizon_sec { 900 => "15m", 3_600 => "1h", 14_400 => "4h", _ => "1d" }.to_string(),
        horizon_sec,
        sample_count: raw,
        ...
        p25_bps: Some(p25),
        p75_bps: Some(p75),
        mfe_bps: Some(blend(weighted_quantile(&mfe, 0.50), model_median.abs() * 1.4)),
        mae_bps: Some(blend(weighted_quantile(&mae, 0.50), -model_median.abs() * 0.9)),
        ...
        baseline_level: if exact > 0 { "exact" } else if raw > 0 { "hierarchical" } else { "model" }.to_string(),
        ...
        sample_from_ts: outcomes.iter().filter(|o| o.horizon_sec == horizon_sec && valid_prior(signal, o)).map(|o| o.event_ts).min(),
        sample_to_ts: outcomes.iter().filter(|o| o.horizon_sec == horizon_sec && valid_prior(signal, o)).map(|o| o.event_ts).max(),
...
pub fn build_hybrid_forecast(
    ...
    let horizons = HORIZONS_SEC.into_iter().map(|horizon_sec| {
        let (stats, raw, effective) = blended_horizon(signal, horizon_sec, historical, transaction_cost, config.prior_strength);
    ...
    let sample_weight = if effective_total <= 0.0 { 0.0 } else { effective_total / (effective_total + config.prior_strength.max(1.0)) };
    ...
    forecast.exact_sample_count = forecast.horizons.iter().map(|h| h.exact_sample_count).max().unwrap_or(0);
    forecast.effective_sample_count = effective_total.round() as usize;
    forecast.maturity_level = maturity;
    forecast.prediction_source = if raw_total == 0 { "model_estimate" } else if effective_total < 300.0 { "hybrid" } else { "historical_validated" }.to_string();
    ...
    forecast.direction_probability = forecast.horizons.first().and_then(|h| h.direction_probability);
    forecast.early_warning = config.dashboard_early_warning_enabled;
    forecast.external_alert_enabled = config.external_directional_alerts_enabled && !config.shadow_mode;
    forecast.prior_strength = config.prior_strength;
    forecast.baseline_level = forecast.horizons.iter().find(|h| h.sample_count > 0).map(|h| h.baseline_level.clone()).unwrap_or_else(|| "model".to_string());
