...
use crate::{
    ...
    toxicity::{
        ...
        toxic_signal_fusion_service::{
            build_toxic_signal_fusion_recent, build_toxic_signal_fusion_status,
        },
    },
    types::{
...
pub struct ToxicSignalFusionQuery {
    symbol: Option<String>,
}

pub async fn toxic_signal_fusion_status(
    State(state): State<AppState>,
    Query(query): Query<ToxicSignalFusionQuery>,
    ...
    Json(serde_json::json!(build_toxic_signal_fusion_status(
        &requested_symbol,
        &active_trade_recent,
        &liquidation_recent,
        &wall_lifecycle_report,
...
pub async fn toxic_signal_fusion_recent(
    State(state): State<AppState>,
    Query(query): Query<ToxicSignalFusionQuery>,
) -> Json<serde_json::Value> {
    let requested_symbol = query
...
pub async fn toxic_signal_fusion_for_symbol(
    State(state): State<AppState>,
...
fn build_report(state: &AppState, requested_symbol: &str) -> ToxicSignalRecentResponse {
    let active_trade_recent = build_active_trade_toxicity_recent(
        requested_symbol,
        &state.flow_state(),
        &state.sweep_state(),
