...
use crate::{
    ...
    toxicity::{
        ...
        toxic_quality_scorecard_service::{
            toxic_quality_scorecard_status, toxic_quality_scorecard_summary,
        },
...
pub struct ToxicQualityScorecardQuery {
    symbol: Option<String>,
}

pub async fn toxic_quality_scorecard_status_route(
    State(state): State<AppState>,
    Query(query): Query<ToxicQualityScorecardQuery>,
...
pub async fn toxic_quality_scorecard_summary_route(
    State(state): State<AppState>,
    Query(query): Query<ToxicQualityScorecardQuery>,
) -> Json<serde_json::Value> {
    let requested_symbol = query
...
pub async fn toxic_quality_scorecard_for_symbol(
    State(state): State<AppState>,
...
) -> crate::types::toxic_quality_scorecard::ToxicQualityScorecardSummaryResponse {
    let fusion_recent = build_fusion_recent(state, requested_symbol);
...
pub(crate) fn build_fusion_recent(
    state: &AppState,
    requested_symbol: &str,
...
pub(crate) fn build_lifecycle_report(
    state: &AppState,
    requested_symbol: &str,
...
pub(crate) fn build_interpretation_report(
    requested_symbol: &str,
