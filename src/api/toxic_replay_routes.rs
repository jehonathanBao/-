...
use crate::{
    ...
    toxicity::{
        ...
        toxic_replay_service::{replay_by_signal_id, replay_latest, replay_recent, replay_status},
    ...
    types::{
        ...
        toxic_replay::{ToxicReplayDetailResponse, ToxicReplayRecentResponse},
        toxic_signal::ToxicSignalRecentResponse,
    },
};

#[derive(Debug, Deserialize)]
pub struct ToxicReplayQuery {
    symbol: Option<String>,
}

pub async fn toxic_replay_status(
    State(state): State<AppState>,
    Query(query): Query<ToxicReplayQuery>,
    ...
    Json(serde_json::json!(replay_status(
        &requested_symbol,
        &fusion_recent,
    )))
}

pub async fn toxic_replay_recent(
    State(state): State<AppState>,
    Query(query): Query<ToxicReplayQuery>,
) -> Json<serde_json::Value> {
    let requested_symbol = query
...
pub async fn toxic_replay_for_symbol(
    State(state): State<AppState>,
...
pub async fn toxic_replay_latest_for_symbol(
    State(state): State<AppState>,
    Path(symbol): Path<String>,
) -> Json<serde_json::Value> {
    Json(serde_json::json!(build_latest_detail(&state, &symbol)))
}

pub async fn toxic_replay_signal_for_symbol(
    State(state): State<AppState>,
    Path((symbol, signal_id)): Path<(String, String)>,
) -> Json<serde_json::Value> {
    Json(serde_json::json!(build_detail_for_signal(
        &state, &symbol, &signal_id,
    )))
}

fn build_recent(state: &AppState, requested_symbol: &str) -> ToxicReplayRecentResponse {
    let fusion_recent = build_fusion_recent(state, requested_symbol);
    replay_recent(requested_symbol, &fusion_recent)
}

fn build_latest_detail(state: &AppState, requested_symbol: &str) -> ToxicReplayDetailResponse {
