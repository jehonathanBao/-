...
use crate::{app::AppState, calibration::manual_apply_runbook::ManualApplyRunbookStore};

pub async fn latest_manual_apply_runbook(State(state): State<AppState>) -> impl IntoResponse {
    let store = runbook_store(&state);
    match store.latest_runbook() {
        Ok(Some(runbook)) => Json(serde_json::json!(runbook)).into_response(),
...
pub async fn manual_apply_runbook_by_id(
    State(state): State<AppState>,
    Path(export_id): Path<String>,
) -> impl IntoResponse {
    let store = runbook_store(&state);
    match store.runbook_by_id(&export_id) {
        Ok(Some(runbook)) => Json(serde_json::json!(runbook)).into_response(),
...
pub async fn latest_manual_apply_runbook_markdown(
    State(state): State<AppState>,
) -> impl IntoResponse {
    let store = runbook_store(&state);
...
pub async fn manual_apply_runbook_markdown_by_id(
    State(state): State<AppState>,
    Path(export_id): Path<String>,
) -> impl IntoResponse {
    let store = runbook_store(&state);
...
fn runbook_store(state: &AppState) -> ManualApplyRunbookStore {
    ManualApplyRunbookStore::new(
