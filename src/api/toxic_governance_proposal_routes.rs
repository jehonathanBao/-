...
use crate::{
    ...
    app::AppState,
    toxicity::toxic_governance_proposal_service::{
        toxic_governance_proposal_export, toxic_governance_proposal_status,
        toxic_governance_proposal_summary,
    },
};

#[derive(Debug, Deserialize)]
pub struct ToxicGovernanceProposalQuery {
    symbol: Option<String>,
}

pub async fn toxic_governance_proposal_status_route(
    State(state): State<AppState>,
    Query(query): Query<ToxicGovernanceProposalQuery>,
...
pub async fn toxic_governance_proposal_summary_route(
    State(state): State<AppState>,
    Query(query): Query<ToxicGovernanceProposalQuery>,
) -> Json<serde_json::Value> {
    let requested_symbol = query
...
pub async fn toxic_governance_proposal_for_symbol(
    State(state): State<AppState>,
...
pub async fn toxic_governance_proposal_export_route(
    State(state): State<AppState>,
    Query(query): Query<ToxicGovernanceProposalQuery>,
) -> Json<serde_json::Value> {
    let requested_symbol = query
...
) -> crate::types::toxic_governance_proposal::ToxicGovernanceProposalSummaryResponse {
    let fusion_recent = build_fusion_recent(state, requested_symbol);
    toxic_governance_proposal_summary(
        requested_symbol,
        &fusion_recent,
